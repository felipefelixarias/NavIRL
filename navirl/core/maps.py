"""Backend-agnostic map loading and spatial metadata.

Provides the shared ``MapInfo`` dataclass, binary occupancy constants, and
``load_map_info`` so that metrics, validators, visualization, and layout
modules can work with *any* backend that uses occupancy-grid maps — not just
``grid2d``.

Re-exported by ``navirl.backends.grid2d.maps`` for backward compatibility.
"""
...
# ---------------------------------------------------------------------------
#  Universal binary-occupancy constants
# ---------------------------------------------------------------------------
