...
def get_backend(name: str, show_warnings: bool = True) -> Callable[..., Any]:
    ...
    if name not in _BACKENDS:
        ...
        raise KeyError(
            ...
            f"{f' (Did you mean \'{suggestion}\'?)' if suggestion else ''}"
        )
...
def _suggest_similar_name(name: str, available: List[str]) -> Optional[str]:
...
def get_plugin_metadata(plugin_type: str, name: str) -> Optional[PluginMetadata]:
    """
