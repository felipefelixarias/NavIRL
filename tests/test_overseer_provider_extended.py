...
class TestEncodeImagesAsDataUrls:
    ...
    def test_skips_nonexistent_file(self, tmp_path):
        urls = _encode_images_as_data_urls([str(tmp_path / "nonexistent.png")], max_images=4)
        assert len(urls) == 0
...
class TestRunOpenaiCompatibleJson:
    ...
    def test_api_non_text_content_raises(self):
        api_response = {"choices": [{"message": {"content": 42}}]}
        cfg = ProviderConfig(
            provider="openai_compatible",
            api_key_env="MOCK_API_KEY",
        )
    ...
    def test_default_endpoint_and_model(self):
        ...
        api_response = {"choices": [{"message": {"content": '{"ok": true}'}}]}
        mock_resp = mock.MagicMock()
        mock_resp.read.return_value = json.dumps(api_response).encode("utf-8")
        mock_resp.__enter__ = mock.MagicMock(return_value=mock_resp)
        mock_resp.__exit__ = mock.MagicMock(return_value=False)
        ...
        with (
            ...
            _run_openai_compatible_json(prompt="test", image_paths=[], schema={}, config=cfg)
...
class TestRunStructuredVlmDispatch:
    def test_native_provider_dispatches(self, tmp_path):
        ...
        script.write_text('import sys, json; json.dump({"ok": True}, sys.stdout)\n')
        cfg = ProviderConfig(
            provider="native",
            native_cmd=f"{sys.executable} {script}",
            timeout_s=10.0,
        )
        result = run_structured_vlm(prompt="test", image_paths=[], schema={}, config=cfg)
